...
export const Route = createLazyFileRoute("/settings/webhooks")({
...
function WebhookActions({ endpoint }: { endpoint: WebhookEndpoint }) {
  ...
  return (
    <div className="flex items-center gap-1">
      <Button variant="ghost" size="sm" onClick={handleTest} disabled={testWebhook.isPending} title={t("send_test")}>
  ...
  return (
    <>
      ...
      <FormDialog
        ...
        onOpenChange={(open) => {
          setCreateOpen(open)
          if (!open) { setNewUrl(""); setNewEvents([]) }
        }}
