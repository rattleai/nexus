...
class Plan(TimestampMixin, Base):
    ...
    tier: Mapped[PlanTier] = mapped_column(
        Enum(PlanTier, values_callable=lambda e: [m.value for m in e]), nullable=False
    )
...
class Subscription(TimestampMixin, Base):
    ...
    status: Mapped[SubscriptionStatus] = mapped_column(
        Enum(SubscriptionStatus, values_callable=lambda e: [m.value for m in e]), default=SubscriptionStatus.ACTIVE
    )
    ...
    __table_args__ = (Index("ix_subscriptions_customer_id", "stripe_customer_id"),)
...
class UsageRecord(Base):
    ...
    __table_args__ = (Index("ix_usage_records_tenant_metric_period", "tenant_id", "metric", "period_start"),)
