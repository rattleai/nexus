...
def main() -> None:
    ...
    with engine.connect() as conn:
        ...
        if result.scalar_one_or_none():
            ...
            conn.execute(text("CREATE ROLE app_user WITH LOGIN PASSWORD 'change-me-in-production'"))
        ...
        conn.execute(
            text("ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT SELECT, INSERT, UPDATE, DELETE ON TABLES TO app_user")
        )
        conn.execute(text("ALTER DEFAULT PRIVILEGES IN SCHEMA public GRANT USAGE, SELECT ON SEQUENCES TO app_user"))
        conn.execute(text("GRANT EXECUTE ON FUNCTION set_config(text, text, boolean) TO app_user"))
        # Audit log immutability (defense-in-depth, idempotent)
        result = conn.execute(text("SELECT 1 FROM information_schema.tables WHERE table_name = 'audit_logs'"))
        if result.scalar_one_or_none():
